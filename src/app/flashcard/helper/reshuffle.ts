...
export const moveCard = (arr, percentage) => {
  if (arr.length > 1) {
    const new_position = Math.max(
      1,
      Math.floor(arr.length * (percentage / 100)),
    );
...
export const next_forgot1_btn = (arr) => {
  ...
  if (arr.length > 1) {
    ...
    if (card.mistakeData.hard > 0) {
      for (
        let i = 1;
        i < card.mistakeData.hard && card.mistakeData.hard > 1;
        i++
      ) {
...
export const next_forgot2_btn = (arr) => {
  ...
  if (arr.length > 1) {
    ...
    if (card.mistakeData.soft > 0) {
      for (
        let i = 1;
        i < card.mistakeData.soft && card.mistakeData.soft > 1;
        i++
      ) {
